...
export function createCaseStudyPanel(caseStudy: CaseStudy, index: number): HTMLElement {
  ...
  const article = el(
    ...
    [
      ordinal,
      el("p", "font-mono text-sm uppercase tracking-[0.3em] text-accent", [caseStudy.tag]),
      ...
      el("div", "grid gap-6 md:grid-cols-2", [
        el("div", "flex flex-col gap-2", [
          el("p", "font-mono text-xs uppercase tracking-[0.2em] text-paper/40", ["Problema"]),
          el("p", "text-base text-paper/75 md:text-lg", [caseStudy.problem]),
        ]),
        el("div", "flex flex-col gap-2", [
          el("p", "font-mono text-xs uppercase tracking-[0.2em] text-paper/40", ["Solución"]),
