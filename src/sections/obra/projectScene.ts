 ...
 * El ordinal gigante hace de telon de fondo con scrub de scroll y los chips
 * usan `rounded-card`, asi que el redondeo cambia con el tema activo.
 */
export function createProjectScene(project: CaseStudy, index: number): HTMLElement {
  const ordinal = el(
    "span",
    "pointer-events-none absolute -top-6 right-0 select-none font-display text-[clamp(7rem,26vw,22rem)] font-black leading-none text-paper/[0.06]",
  ...
  const title = el(
    "h2",
    "mt-5 max-w-3xl font-display text-[clamp(2.2rem,7vw,5.5rem)] font-black uppercase leading-[0.9]",
    [project.title],
  );
  ...
  const chips = el(
    ...
    project.stack.map((tech) => {
      const chip = el(
        "span",
        "rounded-card border border-line bg-paper/[0.04] px-3 py-1.5 font-mono text-[0.65rem] uppercase tracking-[0.15em] text-paper/70",
        [tech],
      );
  ...
  if (project.link) {
    const link = el(
      ...
      [project.link.label, el("span", "transition-transform duration-300 group-hover/link:translate-x-1", ["→"])],
    );
  ...
  return el(
    "section",
    "relative flex min-h-screen flex-col justify-center overflow-hidden border-t border-line px-6 py-24 md:px-12",
    [ordinal, tag, title, body, footer],
  );
}
